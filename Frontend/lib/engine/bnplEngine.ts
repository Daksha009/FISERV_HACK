...
import type { RiskBand } from "./rules";
...
export interface EvaluationResult {
  status: "APPROVED" | "CONDITIONAL" | "REJECTED" | "INVALID_INPUT";
  decision: {
    approved: boolean;
    riskGrade: "A" | "B" | "C" | "D";
    reasonCodes: string[];
...
function pickRiskBand({
  ...
  for (const band of rules.riskBands) {
    if (
      defaults <= band.maxDefaults &&
      creditHistory >= band.minCreditHistory
    ) {
      return band;
    }
  }
...
function validateInput(input: ParsedInput): string[] {
  const errors: string[] = [];
  const fields: (keyof ParsedInput)[] = [
    "monthly_income",
    "credit_history",
    "defaults",
    "product_price",
  ...
  if (Number(input.monthly_income) <= 0)
    errors.push("monthly_income must be > 0");
  if (Number(input.product_price) <= 0)
    errors.push("product_price must be > 0");
  if (Number(input.credit_history) < 0)
    errors.push("credit_history must be >= 0");
...
export function calculateCreditScore(input: ParsedInput): CreditScoreResult {
  const { weights, incomeStability, creditDepth, defaultRecord, debtBurden } =
    rules.scoring;

  // 1. Income Stability Score
  const incomeToProductRatio =
    input.monthly_income / Math.max(input.product_price, 1);
  ...
  const defaultScore = round2(
    clamp(
      defaultRecord.baseScore -
        input.defaults * defaultRecord.penaltyPerDefault,
      0,
      100
    )
  );
  ...
  return {
    ...
    breakdown: {
      incomeStability: {
        ...
        weighted: round2(incomeScore * weights.incomeStability),
      },
      creditDepth: {
        score: creditScore,
        weight: weights.creditDepth,
        weighted: round2(creditScore * weights.creditDepth),
      },
      defaultRecord: {
        score: defaultScore,
        weight: weights.defaultRecord,
        weighted: round2(defaultScore * weights.defaultRecord),
      },
      debtBurden: {
        score: burdenScore,
        weight: weights.debtBurden,
        weighted: round2(burdenScore * weights.debtBurden),
      },
    },
  };
}
...
export function evaluateBnplEligibility(rawInput: RawInput): EvaluationResult {
  ...
  if (validationErrors.length) {
    return {
      ...
      // Provide default values for the rest of the response structure
      decision: { approved: false, riskGrade: "D", reasonCodes: [] },
      creditScore: {
        composite: 0,
        tier: "REJECTED",
        breakdown: {
          incomeStability: { score: 0, weight: 0.25, weighted: 0 },
          creditDepth: { score: 0, weight: 0.25, weighted: 0 },
          defaultRecord: { score: 0, weight: 0.3, weighted: 0 },
          debtBurden: { score: 0, weight: 0.2, weighted: 0 },
        },
      },
      eligibility: { eligibleLimit: 0, productPrice: 0 },
      financials: { processingFee: 0, effectiveAnnualRate: 0 },
      constraints: {
  ...
  if (approvalTier === "CONDITIONAL") {
    ...
    effectiveRate = round2(
      riskBand.annualRate + rules.conditional.rateIncrease
    );
  }
  ...
  const options: EMIOption[] = rules.tenuresInMonths.map((months) => {
    ...
    const affordable =
      emiToIncomeRatio <= rules.affordability.maxEmiToIncomeRatio;
  ...
  let finalStatus: "APPROVED" | "CONDITIONAL" | "REJECTED";
  ...
  const approved =
    finalStatus === "APPROVED" || finalStatus === "CONDITIONAL";
